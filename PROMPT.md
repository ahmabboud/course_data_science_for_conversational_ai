...
>
> Budget **3 to 4 minutes per content slide**, so a 120-minute session in this course is about 16 to 20 slides. Aim for 5 to 8 beats per slide, a beat being one thing you can point at and talk to. A divider before every part, a check question after every concept block. End with the wrap slide and the self-check slide.
...
```
cp lectures/_template.html lectures/dsca-session-01.html
```

Then, in order:

1. **`<body>` attributes**, `data-deck-id` (unique, e.g. `dsca-s01`), `data-course`, `data-session`, `data-duration`, `data-app-root="../"`, `data-unit`.
2. **Title slide**, module tag, eyebrow, title, one-sentence objective, the time split.
3. **Slide skeleton first, prose second.** Lay out ~18 `<section class="slide">` shells with only `data-label`, `data-section` and `data-minutes` filled in, and check the minutes sum to your contact time. Fixing pacing later means rewriting slides.
...
This course's syllabus is `Data Science for Conversational AI - Syllabus.docx`, one directory above the repository, with `CourseObjectives.txt` beside it. The objectives file gives the arc; the docx carries the session breakdown, and each session there already has an objective, a segment table with minutes, a deliverable and a reading. Extract that session's row before you write a slide, and reconcile your `data-minutes` against its segment budget.
