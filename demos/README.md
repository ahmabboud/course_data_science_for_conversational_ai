...
| `module-04/memory_demo.py` | `lectures/dsca-module-04.html` | 5 (Mem0's extract-and-update mechanism) in the Lecture section, 14, 15, 16, 17, 20 (the matching hands-on lab walkthroughs) | The real `mem0ai` package, configured with Gemini for both generation and embeddings and a local on-disk Qdrant store: a restated fact triggers UPDATE not a duplicate ADD, a second `Memory` instance recalls it (standing in for a new session), a long message list gets compacted, and `delete_all` is verified, not just called. |
