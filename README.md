...
- `notebooks/hook_demo.ipynb` is the Module 1 hook, instructor only, not linked from the student-facing index. Run it once before Module 1 with a real connection (a phone hotspot is fine) and save it without clearing output. The naive bot cell needs no connection at all. The reference agent cell does, and its saved output is the safety net if the room's connection has a bad moment during the live session, re-run it live if the connection holds, narrate the saved output if it does not.
